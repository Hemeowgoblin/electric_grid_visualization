...
LINE_DYNAMIC_COLOR = (
    True  # If True, color lines by average HC of connected buses (overrides LINE_COLOR)
)
...
PROPER_LINE_COLOR = (
    "blue"  # Color of proper lines (blue to distinguish from black direct lines)
)
...
SQUARE_ALPHA = (
    1.0  # Transparency of square fill. Range: 0 (invisible) to 1 (fully opaque).
)
...
def line_segments(
    bus: pd.DataFrame,
    line: pd.DataFrame,
    hc_data: pd.DataFrame = None,
    group_hc_df: pd.DataFrame = None,
    line_group_df: pd.DataFrame = None,
    line_group_connectivity_option: int = None,
) -> pd.DataFrame:
    if line.empty:
        return pd.DataFrame()
    bus = bus.rename(columns={"ID": "BusId"})
    b1 = bus[["BusId", "X", "Y"]].rename(
        columns={"BusId": "Bus1", "X": "X1", "Y": "Y1"}
    )
    b2 = bus[["BusId", "X", "Y"]].rename(
        columns={"BusId": "Bus2", "X": "X2", "Y": "Y2"}
    )
    ...
    line_to_group = {}
    if (
        group_hc_df is not None
        and not group_hc_df.empty
        and line_group_df is not None
        and not line_group_df.empty
    ):
...
def proper_line_segments(
    ...
    group_hc_df: pd.DataFrame = None,
    line_group_df: pd.DataFrame = None,
    proper_line_group_connectivity_option: int = None,
    propagated_bus_hc: dict = None,
) -> pd.DataFrame:
    if line.empty:
        return pd.DataFrame()
    ...
    use_propagated_hc = (
        proper_line_group_connectivity_option == 3 and propagated_bus_hc is not None
    )
    ...
    for _, row in line.iterrows():
        ...
        if pd.notna(seg1) and pd.notna(seg2):
            mask = (
                (seg["Line"] == row["ID"]) & (seg["ID"] >= seg1) & (seg["ID"] <= seg2)
            )
...
def find_connected_line_groups_with_switches_and_transformers(
    line: pd.DataFrame, switch: pd.DataFrame, transformer: pd.DataFrame
) -> pd.DataFrame:
    """
    Find groups of electrically connected lines using Union-Find.
...
def calculate_group_hc(
    line_group_df: pd.DataFrame, line: pd.DataFrame, hc_data: pd.DataFrame
) -> pd.DataFrame:
    """
...
def build_entity_bus_mapping(
    line: pd.DataFrame,
    switch: pd.DataFrame,
    transformer: pd.DataFrame,
    cgp: pd.DataFrame,
    use_switches_and_transformers: bool,
) -> dict:
    """
    ...
    if (
        use_switches_and_transformers
        and transformer is not None
        and not transformer.empty
    ):
        for _, row in transformer.iterrows():
...
def propagate_hc_iterative(entity_bus_map: dict, initial_bus_hc: dict) -> dict:
    ...
    for iteration in range(max_iterations):
        ...
        for entity_id, buses in entity_bus_map.items():
            normalized_buses = {
                normalize_bus_id(b) for b in buses if normalize_bus_id(b) is not None
            }
            buses_with_hc = {
                b: hr for b, hr in bus_hc_rank.items() if b in normalized_buses
            }
            ...
            for b in normalized_buses:
                if b in bus_hc_rank:
                    other_buses_with_hc = {
                        bb: hr for bb, hr in buses_with_hc.items() if bb != b
                    }
                    if other_buses_with_hc:
                        other_min_rank = min(
                            hr[1] for hr in other_buses_with_hc.values()
                        )
...
def calculate_line_hc_from_propagated_buses(
    line: pd.DataFrame, bus_hc_rank_dict: dict
) -> pd.DataFrame:
    """
...
def close_file_viewers_windows(file_path: Path):
    ...
    try:
        ...
        if not remaining:
            print(
                "  ✓ File successfully released (no windows with filename in title detected)"
            )
        else:
            print(
                "  ! Some windows still display the file; close them manually if needed"
            )
...
def main():
    ...
    for st_dir in station_dirs:
        ...
        if PLOT_PROPER_LINES and (
            not PROPER_LINE_GROUP_CONNECTIVITY
            or PROPER_LINE_GROUP_CONNECTIVITY_OPTION == 3
        ):
            if PROPER_LINE_GROUP_CONNECTIVITY_OPTION != 3:
                proper_seg = proper_line_segments(
                    bus, line, db_path, dem[["BusId", "HC"]]
                )
    ...
    need_proper_line_grouping = (
        PLOT_PROPER_LINES
        and PROPER_LINE_GROUP_CONNECTIVITY
        and PROPER_LINE_GROUP_CONNECTIVITY_OPTION != 3
    )
    need_proper_line_iterative = (
        PLOT_PROPER_LINES
        and PROPER_LINE_GROUP_CONNECTIVITY
        and PROPER_LINE_GROUP_CONNECTIVITY_OPTION == 3
    )
    ...
    if need_line_grouping and need_proper_line_grouping:
        ...
        lines_for_grouping = all_lines
        use_switches_and_transformers = (
            LINE_GROUP_CONNECTIVITY_WITH_SWITCHES_AND_TRANSFORMERS
        )
    elif need_proper_line_grouping:
        lines_for_grouping = all_lines
        use_switches_and_transformers = (
            PROPER_LINE_GROUP_CONNECTIVITY_WITH_SWITCHES_AND_TRANSFORMERS
        )
    ...
    if lines_for_grouping:
        ...
        all_hc_df = pd.concat(all_hc_data, ignore_index=True).dropna(
            subset=["BusId", "HC"]
        )
        ...
        if use_switches_and_transformers:
            all_switches_df = (
                pd.concat(all_switches, ignore_index=True)
                if all_switches
                else pd.DataFrame()
            )
            all_transformers_df = (
                pd.concat(all_transformers, ignore_index=True)
                if all_transformers
                else pd.DataFrame()
            )
        ...
        if PLOT_LINES and LINE_GROUP_CONNECTIVITY:
            ...
            for st_dir in station_dirs:
                ...
                seg = line_segments(
                    ...
                    group_hc_df,
                    line_group_df,
                    LINE_GROUP_CONNECTIVITY_OPTION,
                )
                if not seg.empty:
                    seg_all.append(seg)
        ...
        if (
            ...
            for st_dir in station_dirs:
                ...
                proper_seg = proper_line_segments(
                    ...
                    group_hc_df,
                    line_group_df,
                    PROPER_LINE_GROUP_CONNECTIVITY_OPTION,
                )
                if not proper_seg.empty:
                    proper_seg_all.append(proper_seg)
        ...
        if need_proper_line_iterative:
            ...
            all_cgps_df = (
                pd.concat(all_cgps, ignore_index=True) if all_cgps else pd.DataFrame()
            )
        ...
        proper_seg = (
            pd.concat(proper_seg_all, ignore_index=True)
            if proper_seg_all
            else pd.DataFrame()
        )
    else:
    ...
    if PLOT_SQUARES:
        square_pts = pd.concat(square_pts_all, ignore_index=True).dropna(
            subset=["X", "Y"]
        )
    ...
    if SAVE_HIGH_RES:
        outpath_hires = (
            outdir
            / f"map_demand_global_hour{CRITICAL_HOUR_DEMAND:02d}_175x132mm_high_resolution.png"
        )
    ...
    if SAVE_SVG:
        outpath_svg = (
            outdir
            / f"map_demand_global_hour{CRITICAL_HOUR_DEMAND:02d}_175x132mm_vector.svg"
        )
    ...
    total_open_switches = sum(
        len(s[(s["State1"] != 1) | (s["State2"] != 1)])
        for s in all_switches
        if not s.empty
    )
    total_closed_switches = sum(
        len(s[(s["State1"] == 1) & (s["State2"] == 1)])
        for s in all_switches
        if not s.empty
    )
